...
class AccountBankStatement(models.Model):
    ...
    def action_bank_reconcile_bank_statements(self):
        self.sudo()
        self.ensure_one()
        self.line_ids.journal_required_if_reconcile = True
        self.line_ids.appear_cancel_button = False
        for line in self.line_ids:
            if not line.journal_bank and self.journal_id.name == 'cheqe':
                raise ValidationError(_('Journal is required for all lines'))

        bank_stmt_lines = self.sudo().mapped('line_ids')
        return {
